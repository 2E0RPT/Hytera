...
static volatile LONG g_transmitting = 0;  // 1 while PTT (Caps Lock) is keyed
...
#define PAYLOAD_OFFSET 28 // CORRECTED from 29 -- confirmed by direct pcap measurement
                           // during HyteraParrot field testing: real downlink packets are
                           // exactly 508 bytes = 28(header) + 480(3x160 audio frames), and
                           // TX/playback packets are 188 = 28+160. With 29 neither divides
                           // evenly by 160. This also settles the long-standing "28 vs 29"
                           // question that was left unresolved in earlier versions.
                           //
                           // NOTE: unlike HyteraParrot, this RX loop was never assuming a
                           // fixed one-frame-per-packet size -- it already decodes and
                           // plays whatever length is present (audio_payload_len), so the
                           // old PAYLOAD_OFFSET=29 wasn't losing 2 of every 3 frames the
                           // way Parrot's bug did. It was still wrong though: every sample
                           // was shifted by one byte and the last byte of each packet's
                           // audio was clipped, a persistent small misalignment on every
                           // single packet rather than catastrophic loss. This fix corrects
                           // that alignment; no other RX logic needed to change.
...
DWORD WINAPI RXThreadFunc(LPVOID lpParam) {
    ...
    while (InterlockedCompareExchange(&g_app_running, 0, 0)) {
        ...
        for (int i = 0; i < audio_payload_len; i++) {
            ...
            // FIX (1-13): skip the float amplify/clamp entirely when RX gain
            // is at its default 1.0x -- mirrors the TX-side fix in 1-12, though
            // note the underlying reason is different here. On TX, skipping at
            // 1.0x avoided a second lossy u-law *re-encode* stacked on the mic
            // driver's own encoding. RX never re-encodes to u-law at all --
            // it decodes once and plays the resulting PCM straight to the
            // sound card -- and an int16 -> float -> int16 round-trip at
            // multiplier 1.0f is exact anyway (a 16-bit int always fits losslessly
            // in a float's 24-bit mantissa), so this isn't fixing an audio-quality
            // bug the way the TX change did. It's just skipping redundant
            // per-sample float math at unity gain.
            if (volume_multiplier != 1.0f) {
    ...
    while (1) {
        ...
        for (int i = 0; i < NUM_AUDIO_BUFFERS; i++) {
            ...
            if (InterlockedCompareExchange(&g_transmitting, 0, 0)) {
                ...
                //
                // FIX (1-12): when gain is at its default 1.0x, skip the decode/
                // re-encode round-trip entirely and pass the mic driver's own
                // u-law bytes straight through. The mic is already opened with
                // WAVE_FORMAT_MULAW_TAG, so Windows has already u-law-encoded
                // this audio once; decoding it back to linear and re-encoding
                // it again (even with no gain change applied) stacks a second
                // lossy quantization pass on top of the first, which was the
                // source of the "tiny bit fuzzy" TX audio reported after the
                // PAYLOAD_OFFSET fix. Comparing tx_gain to 1.0f directly is
                ...
                if (tx_gain == 1.0f) {
                    ...
                    for (DWORD s = 0; s < bytesRecorded; s++) {
                        tx_accumulated_amplitude_sum += abs(ulaw_to_pcm_lut[mic_bytes[s]]);

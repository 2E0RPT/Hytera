...
// --- UDP RSSI Broadcast Protocol (LSIG/ASIG) ---
//
// Broadcasts live and averaged RSSI readings to the whole LAN on port
// 10525, once per telemetry poll, so a separate listener (e.g. a future
// HyteraParrot TTS report) can pick them up over the network without
// being wired directly into this program.
//
// NOTE ON MULTICAST vs BROADCAST: you asked for "multicast" but also
// described plain LAN-wide broadcast + multiple listeners on the same PC.
// Real IP multicast needs a multicast group address (224.0.0.0-
// 239.255.255.255) and listeners to IGMP-join that group -- it's a
// different mechanism to broadcast. What's implemented below is
// broadcast (dest 255.255.255.255), since that's what you actually
// described and it needs no group management. If you really do want
// true multicast (e.g. because this needs to cross a router that blocks
// broadcast), say so and I'll switch the dest address to a multicast
// group and add IGMP group-join on the listener side instead.
#define UDP_TELEMETRY_PORT 10525
#define UDP_BROADCAST_ADDR "255.255.255.255"
#define RSSI_VALID_THRESHOLD (-131) // RSSI must be > this to count as a real signal

static SOCKET g_udpBroadcastSock = INVALID_SOCKET;
static struct sockaddr_in g_udpBroadcastAddr;

// Running-average accumulators, one pair per DMR time slot. Cleared back
// to zero every time an ASIG average gets sent, so they never bleed into
// the next transmission's average.
static double g_ts1_rssi_sum = 0.0;
static int    g_ts1_rssi_count = 0;
static double g_ts2_rssi_sum = 0.0;
static int    g_ts2_rssi_count = 0;

// Opens the UDP socket used to broadcast RSSI telemetry. SO_REUSEADDR is
// set so this socket can still bind to port 10525 even if another
// instance of this program, or a separate listener tool, is already
// bound to that same port on this PC -- without it, the second bind()
// would fail with WSAEADDRINUSE. SO_BROADCAST is required by Winsock
// before a socket is allowed to sendto() a broadcast address at all;
// without it sendto() fails outright with WSAEACCES.
static void init_udp_broadcast_socket(void) {
    if (g_udpBroadcastSock != INVALID_SOCKET) return; // already initialised

    g_udpBroadcastSock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (g_udpBroadcastSock == INVALID_SOCKET) return;

    BOOL reuse = TRUE;
    setsockopt(g_udpBroadcastSock, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuse, sizeof(reuse));

    BOOL bcast = TRUE;
    setsockopt(g_udpBroadcastSock, SOL_SOCKET, SO_BROADCAST, (const char*)&bcast, sizeof(bcast));

    struct sockaddr_in bindAddr;
    memset(&bindAddr, 0, sizeof(bindAddr));
    bindAddr.sin_family = AF_INET;
    bindAddr.sin_addr.s_addr = htonl(INADDR_ANY);
    bindAddr.sin_port = htons(UDP_TELEMETRY_PORT);
    bind(g_udpBroadcastSock, (struct sockaddr*)&bindAddr, sizeof(bindAddr));
    // A bind() failure here is non-fatal: sendto() still works fine on an
    // unbound socket (Winsock just assigns an ephemeral source port), it
    // just won't visibly originate from port 10525 in that fallback case.

    memset(&g_udpBroadcastAddr, 0, sizeof(g_udpBroadcastAddr));
    g_udpBroadcastAddr.sin_family = AF_INET;
    g_udpBroadcastAddr.sin_port = htons(UDP_TELEMETRY_PORT);
    inet_pton(AF_INET, UDP_BROADCAST_ADDR, &g_udpBroadcastAddr.sin_addr);
}

// Sends one "[PREFIX] value\r\n" line to the whole LAN on port 10525.
static void send_udp_rssi_line(const char* prefix, long value) {
    if (g_udpBroadcastSock == INVALID_SOCKET) return;
    char line[64];
    int len = snprintf(line, sizeof(line), "[%s] %ld\r\n", prefix, value);
    sendto(g_udpBroadcastSock, line, len, 0, (struct sockaddr*)&g_udpBroadcastAddr, sizeof(g_udpBroadcastAddr));
}

// Called once per telemetry poll for one DMR slot. shouldSend is decided
// by the caller (see below) -- while it's TRUE, broadcasts a live LSIG
// reading and folds it into that slot's running average. The moment it
// goes FALSE, any readings collected during the just-finished
// transmission are averaged, broadcast as a single ASIG line, and the
// accumulator is reset to zero -- so a channel that's been quiet for a
// while doesn't keep re-sending the same stale average on every
// subsequent poll.
static void process_slot_rssi(long rssi, BOOL shouldSend, const char* lsigPrefix, const char* asigPrefix,
                               double* sum, int* count) {
    if (shouldSend) {
        send_udp_rssi_line(lsigPrefix, rssi);
        *sum += (double)rssi;
        (*count)++;
    } else if (*count > 0) {
        long avg = (long)((*sum / (double)(*count)) + ((*sum >= 0) ? 0.5 : -0.5)); // round to nearest
        send_udp_rssi_line(asigPrefix, avg);
        *sum = 0.0;
        *count = 0;
    }
}
...
unsigned __stdcall TelemetryThread(void* pArguments) {
    ...
    while (1) {
        ...
        if (dataPayload) {
            ...
            // Per your instruction: TS1's messages fire only when TS1 is
            // above threshold, TS2's only when TS2 is above threshold, AND
            // neither fires unless BOTH slots are above threshold at the
            // same poll -- i.e. this is a single combined gate, not two
            // independent ones. A poll where only one slot has a real
            // signal sends nothing for either slot (and flushes whichever
            // slot(s) had a pending average from an earlier both-active
            // stretch, same as if the link had gone offline).
            BOOL ts1_hasSignal = dataPayload->is_online && (dataPayload->ints[7] > RSSI_VALID_THRESHOLD);
            BOOL ts2_hasSignal = dataPayload->is_online && (dataPayload->ints[8] > RSSI_VALID_THRESHOLD);
            BOOL bothSlotsActive = ts1_hasSignal && ts2_hasSignal;

            process_slot_rssi(dataPayload->ints[7], bothSlotsActive,
                               "GUI-TS1-LSIG", "GUI-TS1-ASIG", &g_ts1_rssi_sum, &g_ts1_rssi_count);
            process_slot_rssi(dataPayload->ints[8], bothSlotsActive,
                               "GUI-TS2-LSIG", "GUI-TS2-ASIG", &g_ts2_rssi_sum, &g_ts2_rssi_count);
    ...
    closesocket(snmp_sock);
    closesocket(g_udpBroadcastSock);
    free(params);
    return 0;
}
    ...
    case WM_COMMAND: {
        ...
        if (wmId > 2000 && wmId <= 2050) {
            ...
            if (!receivedFirstData || !currentTelem.is_online) {
                ...
                int newLevel = (currentTelem.tx_power_level == 0) ? 2 : 0;

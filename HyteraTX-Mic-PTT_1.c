...
#include <winsock2.h>
#include <ws2tcpip.h>
#include <windows.h>    // Must come AFTER winsock2.h to avoid winsock.h/winsock2.h clashes.
                        // Needed here for GetAsyncKeyState()/VK_SPACE/VK_ESCAPE (PTT key polling).
...
static HANDLE    g_hStopEvent = NULL;   // Signaled by main thread (Esc key) to end the sender thread
static volatile LONG g_stop_requested = 0;
static volatile LONG g_transmitting   = 0;  // 1 while spacebar is held (PTT keyed), 0 otherwise
...
DWORD WINAPI AudioSenderThread(LPVOID lpParam) {
    ...
    while (1) {
        ...
        for (int i = 0; i < NUM_AUDIO_BUFFERS; i++) {
            ...
            // Only actually transmit while PTT (spacebar) is held down.
            // While not keyed, the buffer is simply recycled -- exactly like a
            // real radio, no RTP stream exists at all when nothing is being sent.
            ...
            // Buffer processed (sent or discarded) -- hand it straight back to the driver
...
int main() {
    ...
    // ==========================================
    // PHASE 1: INITIAL CALL SETUP (also re-sent before every subsequent key-up below,
    // since the repeater appears to tear the call down as soon as PTT de-keys)
    // ==========================================
    ...
    // ==========================================
    // PHASE 2: START MIC CAPTURE (runs continuously; gated by g_transmitting)
    // ==========================================
    ...
    // ==========================================
    // PHASE 3: PUSH-TO-TALK LOOP
    // Hold SPACE to key up and transmit. Release to de-key.
    // Press ESC to end the session entirely.
    // ==========================================
    ...
    while (1) {
        ...
        if (spaceDown && !wasKeyed) {
            printf("[RCP] Re-establishing Call Setup for Talkgroup %d...\n", TARGET_TALKGROUP);
            send_call_setup(rcp_sock, &remote_rcp_addr, CALL_TYPE_GROUP, TARGET_TALKGROUP);
            Sleep(100); // Give the repeater a moment to register the new call before keying up

            printf("[RCP] PTT Key-Up (transmitting)...\n");
            ...
            printf("[RCP] PTT De-Key (idle)...\n");
            send_ptt_command(rcp_sock, &remote_rcp_addr, 0);
            wasKeyed = 0;
        }

        Sleep(10); // Poll roughly every 10ms -- responsive without spinning a full core
    }

    // If the program is being quit while still keyed, make sure we de-key first
    if (wasKeyed) {
        InterlockedExchange(&g_transmitting, 0);

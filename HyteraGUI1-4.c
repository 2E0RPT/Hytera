...
unsigned __stdcall TelemetryThread(void* pArguments) {
    ...
    while (1) {
        ...
        if (dataPayload) {
            ...
            if (n > 0 && parse_telem_getresponse(response, n, dataPayload)) {
                dataPayload->is_online = TRUE;
            } else {
                dataPayload->is_online = FALSE;
            }
...
void PaintAnalogGauge(HDC hdc, int x, int y, int w, int h, const char* name, float value, float minVal, float maxVal, BOOL isAlarm, const char* fmt) {
    ...
    // Intermediate Pop-up Value Text Overlay (22px sized bold graphic typeface centered inside the dial)
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            // Appends the Diagnostic TEMP button layout to the bottom right zone (ID = 5001)
        ...
        case WM_COMMAND: {
            ...
            // Triggered when clicking the diagnostic TEMP button frame
            else if (wmId == 5001) {
                if (!receivedFirstData) {
                    MessageBoxA(hwnd, "Waiting for initial SNMP packet arrival response...", "Diagnostic Notice", MB_OK | MB_ICONINFORMATION);
                } else if (!currentTelem.is_online) {
                    MessageBoxA(hwnd, "Repeater connection state is currently offline.", "Network Status", MB_OK | MB_ICONWARNING);
                } else {
                    char alertText[256];
                    snprintf(alertText, sizeof(alertText), "Raw decoded channel string length: %d\n\nContent string values:\n\"%s\"", 
                             (int)strlen(currentTelem.channel_name), currentTelem.channel_name);
                    
                    MessageBoxA(hwnd, alertText, "SNMP String Value Diagnostic Decoded Output", MB_OK | MB_ICONINFORMATION);
                }
            }
            break;
        }
        ...
        case WM_PAINT: {
            ...
            if (receivedFirstData && currentTelem.is_online) {
                ...
                SetTextColor(hdc, RGB(240, 60, 60)); // Active crimson readout glow
                TextOutA(hdc, 50, 28, strL, (int)strlen(strL));
                
                SetTextColor(hdc, RGB(60, 240, 60)); // Crisp green status accent color
                TextOutA(hdc, 500, 28, "SYSTEM ONLINE", 13);
            } else {
                SetTextColor(hdc, RGB(60, 20, 20)); // Disconnected faded backdrop warning state
                TextOutA(hdc, 50, 28, "S1:---  S2:---", 14);
                
                SetTextColor(hdc, RGB(240, 40, 40)); // Bright critical red error alert
            ...
            SelectObject(hdc, oldFont); DeleteObject(hLedFont); EndPaint(hwnd, &ps);
            break;
        }
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    HWND hwnd = CreateWindowExA(0, "HyteraHardwarePanelClass", "Hytera Telemetry System Hardware Deck",
                                ...
                                CW_USEDEFAULT, CW_USEDEFAULT, 950, 320, NULL, NULL, hInstance, NULL);
    if (!hwnd) return -1;

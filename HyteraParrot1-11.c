...
// ==========================================================
// UDP RSSI RECEIVER -- listens for HyteraGUI's broadcast RSSI protocol
// (HyteraGUI1-18, port 10525, LSIG/ASIG lines like "[GUI-TS1-ASIG] -95")
// and keeps the most recent TS1 average on hand for the spoken feedback
// report. Runs on its own background thread since the broadcasts arrive
// asynchronously and this program's main loop is busy servicing the RTP
// capture/playback socket.
//
// Per your instructions:
//   - only [GUI-TS1-ASIG] is consumed -- live LSIG readings and anything
//     from TS2 are received but deliberately ignored/discarded.
//   - the receive buffer is 8192K (8192*1024 bytes).
//   - the buffer is wiped every time an ASIG value has been used from it.
// ==========================================================
#define UDP_RSSI_LISTEN_PORT 10525
#define UDP_RECV_BUFFER_SIZE (8192 * 1024) // 8192K, per your spec
#define ASIG_TS1_PREFIX "[GUI-TS1-ASIG]"

static char g_udpRecvBuffer[UDP_RECV_BUFFER_SIZE];

// Latest TS1 average RSSI, and whether we've ever actually received one.
// Plain volatile LONG + Interlocked ops rather than a full mutex, since
// it's a single scalar value being handed from the receive thread to the
// main thread -- InterlockedExchange is enough to make that safe.
static volatile LONG g_latestTs1AsigRssi = -200; // -200 sentinel = no ASIG seen yet (matches the repeater's own "no DMR sync" value)
static volatile LONG g_haveTs1AsigValue = 0;

// Timestamp (GetTickCount) of the last ASIG actually received. Needed
// because ASIG is event-driven, not a periodic heartbeat -- HyteraGUI
// only sends one when a transmission ends -- so g_haveTs1AsigValue alone
// can't tell "GUI still running, just quiet for a while" apart from
// "GUI was closed" -- it just stays TRUE forever once set. Comparing
// against this timestamp is what lets a report say "unavailable" again
// once a stored value is too old to trust.
static volatile LONG g_lastAsigReceivedTick = 0;

// How long a stored ASIG value is trusted before being treated as
// unavailable. HyteraGUI's slowest configurable poll interval is 5.0s,
// so this is deliberately a good margin above that -- long enough that
// normal poll-interval/network jitter never falsely flips this, short
// enough that closing HyteraGUI shows up as "unavailable" within a
// reasonable, testable wait rather than lingering indefinitely.
#define RSSI_STALE_TIMEOUT_MS 10000

// Set by the main thread when it wants the receive buffer completely
// wiped (e.g. when going back to "listening for the next transmission").
// The actual memset happens inside UDPRssiReceiveThread itself, not here
// -- reaching into g_udpRecvBuffer directly from the main thread would
// race against the receiver thread's own in-progress recvfrom() into
// that same memory. A flag + a short recv timeout lets the receive
// thread do its own housekeeping safely between packets.
static volatile LONG g_udpBufferWipeRequested = 0;

void request_udp_rssi_buffer_wipe(void) {
    InterlockedExchange(&g_udpBufferWipeRequested, 1);
}

// Discards whatever ASIG value is currently stored, resetting back to
// "none received yet" -- distinct from request_udp_rssi_buffer_wipe()
// above, which only clears the raw network receive buffer. By the time
// an ASIG is spotted, its value has already been pulled out into
// g_latestTs1AsigRssi/g_haveTs1AsigValue (and the underlying buffer
// bytes are typically already self-wiped right after that parse) -- so
// wiping the buffer alone does nothing to undo a premature reading
// that's already been stored. This is what actually makes a stray
// pre-recording "access check" ASIG stop being reused for a report: if
// the real end-of-transmission ASIG hasn't arrived by report time, this
// makes the signal clause correctly say "unavailable" rather than
// silently reusing the stale premature value.
static void invalidate_stored_asig_value(void) {
    InterlockedExchange(&g_haveTs1AsigValue, 0);
    InterlockedExchange(&g_latestTs1AsigRssi, -200);
}

DWORD WINAPI UDPRssiReceiveThread(LPVOID lpParam) {
    SOCKET rssi_sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (rssi_sock == INVALID_SOCKET) {
        printf("[RSSI-UDP] Could not create receive socket -- S-point reporting disabled.\n");
        return 0;
    }

    // SO_REUSEADDR so this can bind port 10525 even though HyteraGUI (the
    // sender) may already be bound to it on this same PC -- same flag
    // HyteraGUI itself sets on its sending socket, for the same reason.
    BOOL reuse = TRUE;
    setsockopt(rssi_sock, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuse, sizeof(reuse));

    // Short receive timeout so this loop wakes up periodically even with
    // no traffic, to check g_udpBufferWipeRequested -- otherwise a wipe
    // request could sit unhandled indefinitely while recvfrom() blocks
    // waiting for a packet that may not come for a while.
    DWORD rx_timeout = 200;
    setsockopt(rssi_sock, SOL_SOCKET, SO_RCVTIMEO, (char*)&rx_timeout, sizeof(rx_timeout));

    struct sockaddr_in bindAddr;
    memset(&bindAddr, 0, sizeof(bindAddr));
    bindAddr.sin_family = AF_INET;
    bindAddr.sin_addr.s_addr = htonl(INADDR_ANY);
    bindAddr.sin_port = htons(UDP_RSSI_LISTEN_PORT);

    if (bind(rssi_sock, (struct sockaddr*)&bindAddr, sizeof(bindAddr)) == SOCKET_ERROR) {
        printf("[RSSI-UDP] bind() on port %d failed (WSAGetLastError=%d) -- S-point reporting disabled.\n",
               UDP_RSSI_LISTEN_PORT, WSAGetLastError());
        closesocket(rssi_sock);
        return 0;
    }

    printf("[RSSI-UDP] Listening for %s on port %d.\n", ASIG_TS1_PREFIX, UDP_RSSI_LISTEN_PORT);

    while (1) {
        // Handle any pending wipe request first, between packets -- this
        // thread is the only writer to g_udpRecvBuffer, so no lock is
        // needed as long as the wipe happens here rather than externally.
        if (InterlockedExchange(&g_udpBufferWipeRequested, 0)) {
            memset(g_udpRecvBuffer, 0, UDP_RECV_BUFFER_SIZE);
            printf("[RSSI-UDP] Receive buffer wiped.\n");
        }

        int n = recvfrom(rssi_sock, g_udpRecvBuffer, UDP_RECV_BUFFER_SIZE - 1, 0, NULL, NULL);
        if (n <= 0) continue; // recv timeout (200ms, no packet) or error -- loop back to re-check the wipe flag
        g_udpRecvBuffer[n] = '\0';

        char* match = strstr(g_udpRecvBuffer, ASIG_TS1_PREFIX);
        if (match) {
            long value = strtol(match + strlen(ASIG_TS1_PREFIX), NULL, 10);
            InterlockedExchange(&g_latestTs1AsigRssi, value);
            InterlockedExchange(&g_haveTs1AsigValue, 1);
            InterlockedExchange(&g_lastAsigReceivedTick, (LONG)GetTickCount());
            printf("[RSSI-UDP] %s received: %ld dB.\n", ASIG_TS1_PREFIX, value);

            // Wipe the buffer now that this ASIG value has been used, per
            // your requirement, so no bytes from this packet can linger
            // into a future (unrelated, smaller) packet's parse.
            memset(g_udpRecvBuffer, 0, UDP_RECV_BUFFER_SIZE);
        }
        // Anything that isn't [GUI-TS1-ASIG] -- live LSIG lines, or
        // either TS2 message -- is received but deliberately left
        // unparsed and unused, per your instructions.
    }

    closesocket(rssi_sock); // unreachable (infinite loop above), kept for symmetry with the rest of this file
    return 0;
}

// ==========================================================
// S-METER CONVERSION -- RSSI (dB) to simulated S-Points.
//
// Ported directly from your FoxPro simulation of the real S-meter
// hardware. Your original comments are preserved verbatim below so the
// calibration reasoning travels with the code.
// ==========================================================
//
// *** The RSSI from the Hytera repeater via SNMP is in the range of -200 No siganl to -131 At an S0 to -58 At S9+40.
// *** The reading was confusing at first, It was a litle bit off, Each real S-Point wasn't 6db (Maybe that's a DMR thing).
// *** The values 131 and 5.3281 was changed from 128 and 5.384. This is calibration in code.
// *** The signal valid starts at -127 anything lower just reports as -200 this represents the loss of DMR sync detect.
// *** Tests carried out by Rob (18/07/2026) Tested on Marconi 2955A last serviced 19/11/2022.
// *** This simulation code is APX 95% accurate to the real hardware, From S3 to S9+30 are almost spot on so it's good enough.
// *** If I was to plot the simulation vs real hardware, The the software has a straight line and the hardware has a slight curve,
// *** Both lines on the plot intersect twice with a minimal divergance from each other.
// *** I tried using the proper db to SPoint formular but it didn't work right. (If ypur reading this, I hope you can do better or accept it as is).
void rssi_to_spoint_string(long rssi_db, char* out, int out_size) {
    // Thisform.label1.Caption = "RSSI = " + STR(This.value) + " db." && It's almots correct.
    double SPoint = 131.0 + (double)rssi_db; // Converts & calibrates reported negative db into positive S-Meter readings.
    if (SPoint > 0.0) {                       // Uses the DMR squelch represented as -200.
        SPoint = SPoint / 5.3281;             // Scaling down both accuracy & scaling for an S-Meter.
        if (SPoint < 10.0) {                  // The 0 to 9 part of the scale.
            int wholeS = (int)(SPoint + 0.5); // Round to nearest whole S-point (was "S %.1f" -- e.g. "S 6.9" -- per your request for a plain integer here).
            if (wholeS > 9) wholeS = 9;        // A value like 9.6 rounds to 10, but "S 10" isn't a real label in
                                                // this scheme -- the >=10 branch below uses "S 9 Plus 10" etc.
                                                // instead, so clamp back to the top of this branch's own scale.
            snprintf(out, out_size, "S %d", wholeS); // S0 to S9.
        } else {
            int whole = (int)SPoint;          // The S9 plus part of the scale.
            if (whole == 10)      snprintf(out, out_size, "S 9 Plus 10");
            else if (whole == 11) snprintf(out, out_size, "S 9 Plus 20");
            else if (whole == 12) snprintf(out, out_size, "S 9 Plus 30");
            else                  snprintf(out, out_size, "S 9 Plus 40"); // whole >= 13, and also catches
                                                                            // anything stronger than your tested
                                                                            // range instead of falling through
                                                                            // unhandled the way a plain ==13
                                                                            // check would -- everything above
                                                                            // S9+40 still reports as S9+40 (top
                                                                            // of scale) rather than staying blank.
        }
    } else {
        snprintf(out, out_size, "NO SIGNAL");
    }
}

// Builds the full spoken clause for TS1's signal, including the
// staleness check -- "Your signal was S 9 Plus 30." if a recent ASIG is
// on hand, or "Signal unavailable." if none has ever arrived, or the
// last one received is older than RSSI_STALE_TIMEOUT_MS (e.g. because
// HyteraGUI has been closed since).
void get_ts1_signal_clause(wchar_t* out, int out_size) {
    DWORD now = GetTickCount();
    DWORD age_ms = now - (DWORD)g_lastAsigReceivedTick; // unsigned subtraction -- wraps correctly even across GetTickCount() rollover, same pattern used elsewhere in this file
    BOOL stale = (!g_haveTs1AsigValue) || (age_ms > RSSI_STALE_TIMEOUT_MS);

    if (stale) {
        _snwprintf(out, out_size, L"Signal unavailable.");
    } else {
        char spointStr[32];
        rssi_to_spoint_string(g_latestTs1AsigRssi, spointStr, sizeof(spointStr));
        wchar_t wSpointStr[32];
        MultiByteToWideChar(CP_ACP, 0, spointStr, -1, wSpointStr, 32);
        _snwprintf(out, out_size, L"Your signal was %s.", wSpointStr);
    }
    out[out_size - 1] = L'\0';
}
...
//
// Signal-strength clause added: reads the latest [GUI-TS1-ASIG] value
// received by UDPRssiReceiveThread and converts it to an S-point via
// rssi_to_spoint_string(). Score is left untouched (still only loss +
// volume, as before) -- you only asked for the S-point to be read out
// in the report, not folded into the numeric score.
void build_feedback_report_text(wchar_t* out_text, int out_text_size, float loss_pct, float* out_score) {
    // 1. Force the local copy to be positive using fabsf()
    float local_loss = fabsf(loss_pct);
    // 2. Perform your upper boundary checks on the local variable
    if (local_loss > 100.0f) local_loss = 100.0f;

    // Loss under 2% is essentially just an artifact of the transmission
    // ending mid-frame at the very last packet, not real lost packets --
    // report that as a clean 0% so people aren't chasing an unreachable
    // 100% score over noise in the measurement itself. The raw [DIAG]
    // line printed in main() still shows the real unfloored number.
    if (local_loss < 2.0f) local_loss = 0.0f;
    ...
    // 3. Use local_loss to calculate the final score
    float score = 100.0f - local_loss - volume_penalty;
    ...
    *out_score = score;
    ...
    wchar_t signalClause[64];
    get_ts1_signal_clause(signalClause, 64);

    // 4. Print local_loss instead of the raw function argument
    _snwprintf(out_text, out_text_size,
        L"%s Your audio was %s, and you have %.0f percent packets lost. Your overall score is %.0f percent.",
        signalClause, wVolumeDesc, local_loss, score);
...
int main(int argc, char *argv[]) {
    ...
    // Fire-and-forget background thread, same pattern as HyteraGUI's own
    // TelemetryThread -- runs for the life of the program, never joined.
    CreateThread(NULL, 0, UDPRssiReceiveThread, NULL, 0, NULL);
    ...
    while (1) {
        ...
        if (bytes_received > PAYLOAD_OFFSET) {
            ...
            if (!recording) {
                ...
                // Occasionally the repeater/DMR access process sends one
                // premature [GUI-TS1-ASIG] right before real audio starts
                // (believed to be a repeater-access check, not a genuine
                // end-of-transmission average). Wiping the raw network
                // buffer alone isn't enough to undo it though -- by now
                // its value has already been parsed out into the stored
                // globals -- so also invalidate the stored value itself.
                // The real ASIG for this transmission arrives when the
                // user de-keys, and will be more accurate anyway from the
                // larger averaged sample.
                request_udp_rssi_buffer_wipe();
                invalidate_stored_asig_value();
            } else {
        ...
        if (recording && (now - last_packet_time) >= SILENCE_TIMEOUT_MS) {
            ...
            // Back to "Listening for the next transmission" -- per your
            // request, totally erase the port-10525 RSSI receive buffer
            // at this point so nothing from the just-finished
            // transmission's telemetry can linger into the next one.
            request_udp_rssi_buffer_wipe();
        }
    }

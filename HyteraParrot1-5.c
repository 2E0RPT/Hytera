...
const GUID CLSID_SpStream      = {0xAF35D1D3, 0x8447, 0x4632, {0xA3,0x22,0x53,0xE9,0x65,0x45,0x30,0x1C}};
const GUID IID_ISpStream       = {0xCF3D2E50, 0x53F2, 0x11D2, {0x9E,0xE7,0x00,0xC0,0x4F,0x79,0x73,0x96}};
...
#define VOL_THRESH_VERY_QUIET   500.0f
#define VOL_THRESH_QUIET       2000.0f
#define VOL_THRESH_GOOD        5000.0f
#define VOL_THRESH_LOUD        8000.0f
...
#define SCORE_PENALTY_VOLUME_MAJOR 10.0f
...
const char* classify_volume(float avg_volume) {
    if (avg_volume < VOL_THRESH_VERY_QUIET) return "very quiet";
    if (avg_volume < VOL_THRESH_QUIET)      return "quiet";
    if (avg_volume < VOL_THRESH_GOOD)       return "a good level";
    if (avg_volume < VOL_THRESH_LOUD)       return "slightly loud";
    return "very loud";
}
...
void build_feedback_report_text(wchar_t* out_text, int out_text_size, float loss_pct, float* out_score) {
    ...
    *out_score = score;
    ...
    _snwprintf(out_text, out_text_size,
        L"Your audio was %s, with %.0f percent packets lost. Your overall score is %.0f percent.",
...
int synthesize_report_to_frames(const wchar_t* text) {
    ...
    if (rawWav && totalBytes > 12 && memcmp(rawWav, "RIFF", 4) == 0 && memcmp(rawWav + 8, "WAVE", 4) == 0) {
        // Walk RIFF chunks to find "data" -- don't assume a fixed header
        // size, since SAPI's exact chunk layout isn't something we've
        // verified byte-for-byte the way other formats in this project
        // have been.
        uint32_t offset = 12;
        ...
        while (offset + 8 <= totalBytes) {
            char chunkId[5] = {0};
            memcpy(chunkId, rawWav + offset, 4);
            uint32_t chunkSize;
            memcpy(&chunkSize, rawWav + offset + 4, 4);
            uint32_t chunkDataStart = offset + 8;
            if (memcmp(chunkId, "data", 4) == 0) {
                dataPtr = rawWav + chunkDataStart;
                dataLen = chunkSize;
                break;
            }
            offset = chunkDataStart + chunkSize + (chunkSize & 1); // chunks are word-aligned
        }

        if (dataPtr && dataLen >= 2) {
            int16_t* pcm16 = (int16_t*)dataPtr;
            uint32_t numSamples = dataLen / 2;
            ...
            if (frames_written == 0) {
                // This used to be a silent failure path -- outSampleCount
                // could end up 0 (e.g. numSamples too small) with nothing
                // printed at all. Now it can't hide.
                printf("[TTS] Resampling produced 0 frames from %u raw samples -- skipping spoken report.\n", numSamples);
            }
        } else {
            printf("[TTS] Could not find a data chunk in SAPI's output (dataPtr=%p, dataLen=%u) -- skipping spoken report.\n", (void*)dataPtr, dataLen);
        }
    } else {
        printf("[TTS] SAPI output wasn't a valid WAV (or was empty, totalBytes=%lu) -- skipping spoken report.\n", totalBytes);
    }
